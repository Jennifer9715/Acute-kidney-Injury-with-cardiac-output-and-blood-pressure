...
```{r}
mean_age <- mean(age$AGE)
std_age <- sd(age$AGE)
range_age <- range(age$AGE)
```
...
age_freq <- data.frame(table(age$AGE))
...
```{r}
hospital_freq <- data.frame(table(hospital_data_timefreq$hospital_admission_freq))
...
icu_freq <- data.frame(table(icu_data_timefreq$icu_stay_freq))
...
ggsave(filename="icu_freq_plott.jpg", plot=icu_freq_plot)
```
